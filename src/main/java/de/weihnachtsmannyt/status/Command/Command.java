...
public class Command implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            switch (args[0].toLowerCase()) { //status ___
                ...
                case "set" -> {
                    Boolean UppercaseLengthLimitToggle = Status.getInstance().getConfig().getBoolean("Uppercase/LengthLimit-Toggle");
                    switch (args.length){
                        case 2 ->{
                        ...
                        case 4 ->{
                            ...
                            } else p.sendMessage(Status.getInstance().getStatus_Prefix() +
                        ...
                        case 2 -> {
                            ...
                            if (target.hasPlayedBefore()) {
                                ...
                                if (PrefixManager.isColorAColor(color)) {
                                    ...
                                    p.sendMessage(Status.getInstance().getStatus_Prefix() + "§7Der Spieler wurde nicht gefunden!");
                                }
                            } else {
                                p.sendMessage(Status.getInstance().getStatus_Prefix() + "§7Der Spieler wurde nicht gefunden!");
                            }
    ...
    private void sendFailedCmd(CommandSender sender) {
        sender.sendMessage(Status.getInstance().getStatus_Prefix()+"§c§l Failed, Wrong Cmd or Token!");
    }
}